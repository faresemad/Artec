...
class StudentModelTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(email='testuser@example.com', name='Test User', password='password')
...
class McqAnswerModelTestCase(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(email='test@example.com', password='test1234', name='Test User')
        self.student = Student.objects.create(user=self.user, full_name='Test Student', student_photo='test.jpg', national_id='12345678901234', seat_number=1, total=50.00, division='1', phone_number='1234567890', college=self.college)
        self.exam = MCQExam.objects.create(question='Test Question', option1='Option 1', option2='Option 2', option3='Option 3', answer='Option 1', college=self.college)
        self.answer = McqAnswer.objects.create(student=self.student, question=self.exam, answer='Option 1', is_correct=True, score=5)
...
class HandDrawingAnswerModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.hand_drawing_answer = HandDrawingAnswer.objects.create(
            ...
            answer=SimpleUploadedFile("test_hand_drawing_answer.jpg", b"file_content", content_type="image/jpeg"),
            score=0
        )
    ...
    def test_hand_drawing_answer_str(self):
        ...
        hand_draw=self.hand_drawing_exam,
        answer="test.png"
    )
...
class DigitalDrawingAnswerModelTest(TestCase):
    ...
    def setUp(self):
        self.college = College.objects.create(name="Test College", payment_code="12345")
        self.user = User.objects.create_user(email="test@example.com", name="Test User", password="testpass")
        self.student = Student.objects.create(user=self.user, full_name="Test Student", national_id="12345678901234",
                                            seat_number=100, total=80.0, division="1", phone_number="123456789",
                                            college=self.college)
        self.drawing_exam = DigitalDrawingExam.objects.create(question="Test Drawing Exam", task_description="Draw a circle", college=self.college)
        self.image = SimpleUploadedFile("test_image.jpg", content=b"file_content", content_type="image/jpg")
        self.digital_drawing_answer = DigitalDrawingAnswer.objects.create(student=self.student, digital_draw=self.drawing_exam, answer=self.image, score=0)
    ...
    def test_digital_drawing_answer_answer(self):
        ...
        self.assertEqual(generated_name, expected_name, "The names of the image files should be equal")
...
class PracticeDrawingAnswerTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.student = Student.objects.create(
            user=cls.user, full_name='Test Student', student_photo=SimpleUploadedFile(
                'test_photo.jpg', b'content'), national_id='12345678901234', seat_number=1, total=100.0, division='1', phone_number='123456789', college=cls.college)
        cls.answer = PracticeDrawingAnswer.objects.create(
            student=cls.student, practice_draw=cls.exam, answer=SimpleUploadedFile('test_answer.jpg', b'content'), score=50)
